...
def test_actor_create(api_client, canada):
    ...
    response = api_client.post('/api/v1/actors/', payload, format='json')
...
def test_retrieve_actor_list(api_client):
    response = api_client.get('/api/v1/actors/', format='json')
...
def test_actor_detail_not_found(api_client):
    response = api_client.get('/api/v1/actors/1/', format='json')
    assert response.status_code == HTTPStatus.NOT_FOUND


@pytest.mark.django_db
def test_retrieve_actor_detail(api_client, actor):
    response = api_client.get(f'/api/v1/actors/{actor.id}/', format='json')
    assert response.status_code == HTTPStatus.OK
...
def test_actor_update(api_client, actor):
    ...
    response = api_client.put(
        f'/api/v1/actors/{actor.id}/', payload, format='json'
    )
...
def test_actor_delete(api_client, actor):
    response = api_client.delete(f'/api/v1/actors/{actor.id}/', format='json')
