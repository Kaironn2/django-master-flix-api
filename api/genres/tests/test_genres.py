...
def test_genre_create(api_client):
    payload = {'name': 'action'}
    response = api_client.post('/genres/', payload, format='json')
...
def test_retrieve_genre_list(api_client, action_genre):
    response = api_client.get('/genres/', format='json')
...
def test_genre_detail_not_found(api_client):
    response = api_client.get('/genres/1/', format='json')
    assert response.status_code == HTTPStatus.NOT_FOUND


@pytest.mark.django_db
def test_retrieve_genre_detail(api_client, action_genre):
    response = api_client.get(f'/genres/{action_genre.id}/', format='json')
    assert response.status_code == HTTPStatus.OK
...
def test_genre_update(api_client, action_genre):
    ...
    response = api_client.put(
        f'/genres/{action_genre.id}/', payload, format='json'
    )
...
def test_genre_delete(api_client, action_genre):
    response = api_client.delete(f'/genres/{action_genre.id}/', format='json')
